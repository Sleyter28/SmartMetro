...
public class Aggregator extends MIDlet {
    ...
    private RadiogramConnection connRain;
    private Datagram datagramRain;
    private String receiveValueSink;
    private int voltage;
    ...
    protected void startApp() throws MIDletStateChangeException {
       System.out.println("Hello Aggregator");
        try {
            ...
            connRain = (RadiogramConnection)Connector.open("radiogram://7f00.0101.0000.1005:89"); // send data to the rain node
            datagramRain = (Datagram) connRain.newDatagram(connRain.getMaximumLength());
           ...
           while (true) {
               try {
                   ...
                   if (conn.packetsAvailable()) {
                       ...
                       receiveValueSink = datagram.readUTF();
...
//                  System.out.println("Caida en vias");
              ...
              if (receiveValueSink.startsWith("true")) {
                  //Send metroIn to light node
                  metroIn= true;
                  dg2.writeBoolean(metroIn);
                  conn2.send(dg2);
                  dg2.reset();

                  //Send metroIn to accelerometer
                  ...
                  //Change threshold value of the voltage (rain sensor)
                  voltage = Integer.parseInt(receiveValueSink);
                  datagramRain.writeInt(voltage);
                  connRain.send(datagramRain);
                  datagramRain.reset();
                  receiveValueSink = "";
              }
              metroIn = false;
